...
function nextFrame(timeStamp) {
  ...
  if(fade.phase==1){
   fade.time=Math.max(fade.time-dt/1000,0)
   if(fade.time==0)fade.phase=2
  }
  if(fade.phase==0){
   fade.time=Math.min(fade.time+dt/200,2)
   if(fade.time==2)fade.phase=1
  }
  document.body.style.opacity = (100-fade.time*50)+"%"
...
let starting_int = setInterval(()=>{
 if(difficulties[2].level){
  load();
  clearInterval(starting_int)
  fade.phase = 1
  respawn(false);
  adjustScreen(true);
  window.requestAnimationFrame(nextFrame);
